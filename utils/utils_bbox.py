...
def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape, letterbox_image):
    ...
    Func:
        ...
        对不失真resize的bbox进行调整
    Args:
        box_xy:      归一化的bbox的中心点坐标 tensor shape(K, 2) 
        box_wh:      归一化的bbox的宽高wh
        input_shape: 输入的形状
        image_shape: 图片的形状
        letterbox_image: 是否使用padding resize
    Returns:
        boxes:      是左上右下的真实坐标
                    tensor shape(K, 4), 4 => (y1, x1, y2, x2)
    """
    ...
    if letterbox_image:
        #   offset是图像有效区域相对于图像左上角的偏移情况,值在0~1之间
        #   new_shape指的是宽高缩放情况
...
def DecodeBox(outputs,
    ...
    #   相当于网络结构中Prediction的Transpose
    #   outputs是个list[]包含三个head的输出
    #   (batch_size, 80, 80, 5 + num_classes)   ->  (batch_size, 6400, 5 + num_classes)
    #   (batch_size, 40, 40, 5 + num_classes)   ->  (batch_size, 1600, 5 + num_classes)
    #   (batch_size, 20, 20, 5 + num_classes)   ->  (batch_size,  400, 5 + num_classes)
    #   outputs  shape(batch_size, 8400, 5 + num_classes)
    ...
    #   获得真实的bbox大小
    #   将box_xy和box_wh调节成boxes=(y_min,x_min,y_max,x_max) shape(K, 4)
...
if __name__ == "__main__":
    ...
    def decode_for_vision(output):
        ...
        # batch_size, 20, 20, 5 + num_classes
        bs, hw = np.shape(output)[0], np.shape(output)[1:3]
        # batch_size, 400, 5 + num_classes
